...
class TrimParameterSet(object):
    ...
    def calculateScore(self):
        return (self.readRetention * 100) - (1 * (((self.forwardMaxExpectedError - 1)**2) + ((self.reverseMaxExpectedError - 1)**2))) # - (0.025 * abs(self.forwardTrimPosition - self.reverseTrimPosition))
...
def getFastqList(path:str):
    return formatReaders.fastq.fastqHandler.findSamplesInFolder(path)
...
class Q2ArrayParallelBuilderAgent(object):
    ...
    def makeQ2Array(self, fastqFileInfo: formatReaders.fastq.fileNamingStandards.NamingStandard):
        import numpy
        #print("Running %s" %fastq)
        fastq = formatReaders.fastq.fastqHandler.FastqFile(fastqFileInfo.filePath, depth=1, subsample=self.subsample)
        q2Locations = []
        for read in fastq:
            containedQ2 = False
...
def makeCombinedQ2ArrayForOneDirection(fastqList:list, sampleOrder:list, subsample):
    import numpy
    from ... import easyMultiprocessing
    parallelBuildAgent = Q2ArrayParallelBuilderAgent(subsample)
...
def makeCombinedQ2ArraysForBothEnds(fastqList:list, sampleOrder:list, subsample):
    ...
    forwardQ2Array = makeCombinedQ2ArrayForOneDirection(forwardFastqList, sampleOrder, subsample)
    reverseQ2Array = makeCombinedQ2ArrayForOneDirection(reverseFastqList, sampleOrder, subsample)
...
class NBaseArrayParallelBuilderAgent(object):
    ...
    def makeFirstNBaseArray(self, fastqFileInfo: formatReaders.fastq.fileNamingStandards.NamingStandard):
        import numpy
        #print("Running %s" %fastq)
        fastq = formatReaders.fastq.fastqHandler.FastqFile(fastqFileInfo.filePath, subsample=self.subsample)
        nBaseLocations = []
        for read in fastq:
            containedN = False
...
def makeCombinedFirstNBaseArrayForOneDirection(fastqList:list, sampleOrder:list, subsample:int):
    import numpy
    from ... import easyMultiprocessing
    parallelBuildAgent = NBaseArrayParallelBuilderAgent()
...
def makeCombinedFirstNBaseArraysForBothEnds(fastqList:list, sampleOrder:list, subsample:int):
    forwardFastqList = [fastq for fastq in fastqList if fastq.direction == 1]
    reverseFastqList = [fastq for fastq in fastqList if fastq.direction == 2]
    forwardFirstNBaseArray = makeCombinedFirstNBaseArrayForOneDirection(forwardFastqList, sampleOrder, subsample)
    reverseFirstNBaseArray = makeCombinedFirstNBaseArrayForOneDirection(reverseFastqList, sampleOrder, subsample)
...
class ExpectedErrorMatrixBuilderParallelAgent(object):
    ...
    def __init__(self, startPosition:int = 0, subsample:int=0):
        self.startPosition = startPosition
        self.subsample = subsample

    def makeExpectedErrorMatrix(self, fastq: formatReaders.fastq.fileNamingStandards.NamingStandard):
        #print("Running %s" %fastq)
        expectedErrorMatrix = formatReaders.fastq.fastqAnalysis.buildExpectedErrorMatrix(fastq.filePath, superLean=True,
            startPosition=self.startPosition, subsample=self.subsample)  # Transpose later, keeping this order should allow for faster concatenation later.
        return fastq, expectedErrorMatrix


def makeCombinedExpectedErrorMatrixForOneDirection(fastqList:list, sampleOrder:list, subsample:int, startPosition:int = 0):
    import numpy
    from ... import easyMultiprocessing
    parallelBuildAgent = ExpectedErrorMatrixBuilderParallelAgent(startPosition, subsample)
...
def makeCombinedErrorMatricesForBothEnds(fastqList:list, sampleOrder:list, subsample:int, minimumTrimPositions:tuple = (0,0)):
    forwardMinimumTrimPosition, reverseMinimumTrimPosition = minimumTrimPositions
    forwardFastqList = [fastq for fastq in fastqList if fastq.direction == 1]
    reverseFastqList = [fastq for fastq in fastqList if fastq.direction == 2]
    forwardExpectedErrorMatrix = makeCombinedExpectedErrorMatrixForOneDirection(forwardFastqList, sampleOrder, subsample, forwardMinimumTrimPosition)
    reverseExpectedErrorMatrix = makeCombinedExpectedErrorMatrixForOneDirection(reverseFastqList, sampleOrder, subsample, reverseMinimumTrimPosition)
...
def runTrimParameterTest(forwardExpectedErrorMatrix:numpy.ndarray, reverseExpectedErrorMatrix:numpy.ndarray, forwardFirstNBaseArray:numpy.ndarray, reverseFirstNBaseArray:numpy.ndarray, forwardQ2Array:numpy.ndarray, reverseQ2Array:numpy.ndarray, trimPositions:tuple, minimumTrimPositions:tuple = (0, 0), forwardCurve:expectedErrorCurve.ExponentialFit=None, reverseCurve:expectedErrorCurve.ExponentialFit=None):
    ...
    for forwardTrimPosition, reverseTrimPosition in trimPositions:
        ...
        keptReads = 0
        rejectedReads = 0
        for forwardExpectedErrorValue, reverseExpectedErrorValue, forwardFirstNBasePosition, reverseFirstNBasePosition, forwardQ2Position, reverseQ2Position in zip(forwardExpectedErrors[0], reverseExpectedErrors[0], forwardFirstNBaseArray, reverseFirstNBaseArray, forwardQ2Array, reverseQ2Array):
            totalReads += 1
            if forwardExpectedErrorValue >= forwardMaxExpectedError or reverseExpectedErrorValue >= reverseMaxExpectedError: #Using this because I lose fractional values to save on memory. In theory, this would probably disagree with dada2's decision if the real value is exactly an integer with no fractional portion.  This is unlikely to happen often enough to be an issue.
                ...
                keptReads += 1
        results.append(TrimParameterSet(forwardTrimPosition + 1, reverseTrimPosition + 1, forwardMaxExpectedError, reverseMaxExpectedError, keptReads/totalReads)) #doing +1 to adjust for zero indexed matrices
        results.sort(key = operator.attrgetter("score"), reverse=True)
    return results


def runTrimParameterTestLite(forwardExpectedErrorMatrix:numpy.ndarray, reverseExpectedErrorMatrix:numpy.ndarray, trimPositions:tuple, minimumTrimPositions:tuple = (0, 0), forwardCurve:expectedErrorCurve.ExponentialFit=None, reverseCurve:expectedErrorCurve.ExponentialFit=None):
    import operator
    forwardMinimumTrimPosition, reverseMinimumTrimPosition = minimumTrimPositions
    results = []
    for forwardTrimPosition, reverseTrimPosition in trimPositions:
...
def parallelReadLengthChecker(fastq:formatReaders.fastq.fileNamingStandards.NamingStandard):
    return fastq, formatReaders.fastq.fastqHandler.estimateReadLength(fastq.filePath, getVariance=True)
...
def checkReadLengths(fastqList:list):
    ...
    if not filesPassCheck:
        raise formatReaders.fastq.fastqHandler.FastqValidationError("Unable to validate fastq files enough to perform this operation. Please check log for specific error(s).")
    return read1Length, read2Length


def performAnalysis(inputDirectory:str, minimumCombinedReadLength:int, subsample:int=0, percentile:int=83, fastqList:list = None, makeExpectedErrorPlots:bool = True):
    from . import expectedErrorCurve
    ...
    forwardCurve, reverseCurve = expectedErrorCurve.calculateExpectedErrorCurvesForFastqList(fastqList, subsample=subsample, percentile=percentile, makePNG=makeExpectedErrorPlots)
    ...
    forwardQ2Array, reverseQ2Array = makeCombinedQ2ArraysForBothEnds(fastqList, sampleOrder, subsample)
    forwardFirstNBaseArray, reverseFirstNBaseArray = makeCombinedFirstNBaseArraysForBothEnds(fastqList, sampleOrder, subsample)
    forwardExpectedErrorMatrix, reverseExpectedErrorMatrix = makeCombinedErrorMatricesForBothEnds(fastqList, sampleOrder, subsample, minimumTrimmingPositions)
    resultTable = runTrimParameterTest(forwardExpectedErrorMatrix, reverseExpectedErrorMatrix, forwardFirstNBaseArray, reverseFirstNBaseArray, forwardQ2Array, reverseQ2Array, trimPositions, minimumTrimmingPositions, forwardCurve, reverseCurve)
    return resultTable, forwardCurve, reverseCurve


def performAnalysisLite(inputDirectory:str, minimumCombinedReadLength:int, subsample:int=0, percentile:int=83, fastqList:list = None, makeExpectedErrorPlots:bool = True):
    from . import expectedErrorCurve
    if not inputDirectory:
        if not fastqList:
            raise ValueError("No input directory and no fastq list were given.")
    ...
    trimPositions = makeAllPossibleTrimLocations(forwardReadLength, reverseReadLength, minimumCombinedReadLength)
    forwardExpectedErrorMatrix, reverseExpectedErrorMatrix = makeCombinedErrorMatricesForBothEnds(fastqList, sampleOrder, subsample, minimumTrimmingPositions)
    resultTable = runTrimParameterTestLite(forwardExpectedErrorMatrix, reverseExpectedErrorMatrix, trimPositions, minimumTrimmingPositions, forwardCurve, reverseCurve)
    return resultTable, forwardCurve, reverseCurve
